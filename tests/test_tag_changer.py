...
# Фикстуры отдельно вынести
# Остались ещё функции: delete_images, get_info_from_file, change_tags
...
class TestTagChanger:
    ...
    target_dir = '/mnt/c/code/tag_changer/test_target_dir/'
    datas_11 = [
    ...
    def tag_changer(self):
        ...
        tc = TagChanger()
        tc.target_dir = '/mnt/c/code/tag_changer/test_target_dir/'
        tc.artist_dirs = ['Легенды', 'Legend']
        return tc
    ...
    def test_get_image_and_delete_images(self, tag_changer, input, output):
        # TODO: дописать тесты через вывод логов
        assert tag_changer.get_image(*input) == output
        # tag_changer.delete_images(tag_changer.target_dir)
